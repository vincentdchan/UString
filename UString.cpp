...
std::vector<UString> UString::split(const UString &sep) const {
    ...
    while ((end = find_string(this->constData(), this->length(), start, sep.constData(), sep.length())) != -1) {
        if (start != end) {
...
size_t UStringHashBits(const void *p, size_t size, size_t seed) noexcept
{
    ...
    return siphash(static_cast<const uint8_t*>(p), size, seed);
}
#endif
